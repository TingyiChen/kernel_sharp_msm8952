...
static size_t shdisp_dbg_get_stacktrace(char *stackdumpbuf, size_t length)
{
    ...
    SHDISP_TRACE("ptr=0x%p len=%ld", stackdumpbuf, (long)length);
    while (1) {
        int urc, rtn;
    ...
    SHDISP_TRACE("dump stuck trace size=%ld", (long)writelen);
    return writelen;
}
...
{
    if (subcode_val != SHDISP_DBG_SUBCODE_NONE && sub_code != SHDISP_DBG_SUBCODE_NONE) {
        return SHDISP_RESULT_FAILURE;
    }
