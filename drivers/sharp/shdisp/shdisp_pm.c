...
struct shdisp_pm_context {
    ...
    struct shdisp_bdic_status  panel_status;
...
#if defined(USE_LINUX) || defined(SHDISP_APPSBL)
static int shdisp_pm_panel_set_active(int mode);
static int shdisp_pm_panel_set_standby(int mode);
...
struct shdisp_bdic_status lpsoc_status;
...
void shdisp_pm_API_init(struct shdisp_boot_context *shdisp_boot_ctx)
{
    ...
    shdisp_pm_ctx.panel_status.power_status = SHDISP_DEV_STATE_OFF;
    shdisp_pm_ctx.panel_status.users = SHDISP_DEV_TYPE_NONE;
    ...
    if (shdisp_api_get_boot_disp_status() == SHDISP_MAIN_DISP_ON) {
        shdisp_pm_ctx.panel_status.power_status = SHDISP_DEV_STATE_ON;
        shdisp_pm_ctx.panel_status.users |= SHDISP_DEV_TYPE_LCD;
    }
    ...
    lpsoc_status.power_status  = SHDISP_DEV_STATE_OFF;
    lpsoc_status.users         = SHDISP_DEV_TYPE_NONE;
}
...
/* ------------------------------------------------------------------------- */
/* shdisp_pm_API_lpsoc_power_manager                                         */
/* ------------------------------------------------------------------------- */
int shdisp_pm_API_lpsoc_power_manager(int user, int onoff)
{
    unsigned long users_wk;

    SHDISP_TRACE("in  [LPSOC_PM]  users:0x%08X, power_status:%s, user:0x%08X, onoff:%s",
            (int)lpsoc_status.users,
            GET_DEV_STATE_STR(lpsoc_status.power_status),
            user,
            GET_DEV_REQ_STR(onoff));

    if ((user != (user & SHDISP_DEV_TYPE_LPSOC_MASK)) || (user == SHDISP_DEV_TYPE_NONE)) {
        SHDISP_ERR("invalid user argument. user:0x%08X", user);
        return SHDISP_RESULT_FAILURE;
    }
    ...
    if (onoff == SHDISP_DEV_REQ_ON) {
        ...
        case SHDISP_DEV_STATE_OFF:
            ...
            lpsoc_status.power_status = SHDISP_DEV_STATE_ON;
            break;
        }
    } else if (onoff == SHDISP_DEV_REQ_OFF) {
        switch (lpsoc_status.power_status) {
        case SHDISP_DEV_STATE_ON:
            ...
            users_wk &= (unsigned long)(~(user & SHDISP_DEV_TYPE_LPSOC_MASK));
            if (users_wk == SHDISP_DEV_TYPE_NONE) {
    ...
    if (onoff == SHDISP_DEV_REQ_ON) {
        lpsoc_status.users |= (unsigned long)(user & SHDISP_DEV_TYPE_LPSOC_MASK);
    } else if (onoff == SHDISP_DEV_REQ_OFF) {
        lpsoc_status.users &= (unsigned long)(~(user & SHDISP_DEV_TYPE_LPSOC_MASK));
    }


    SHDISP_TRACE("out [LPSOC_PM]  users:0x%08X, power_status:%s",
            (int)lpsoc_status.users,
            GET_DEV_STATE_STR(lpsoc_status.power_status));
...
#if defined(USE_LINUX) || defined(SHDISP_APPSBL)
/* ------------------------------------------------------------------------- */
/* shdisp_pm_API_panel_power_manager                                         */
/* ------------------------------------------------------------------------- */
int shdisp_pm_API_panel_power_manager(int user, int onoff, int mode)
{
    ...
    SHDISP_TRACE("in  [PANEL_PM]  users:0x%08X, power_status:%s, user:0x%08X, onoff:%s",
            (int)shdisp_pm_ctx.panel_status.users,
            GET_DEV_STATE_STR(shdisp_pm_ctx.panel_status.power_status),
            user,
            GET_DEV_REQ_STR(onoff));

    if ((user != (user & SHDISP_DEV_TYPE_PANEL_MASK)) || (user == SHDISP_DEV_TYPE_NONE)) {
        SHDISP_ERR("invalid user argument. user:0x%08X", user);
        return SHDISP_RESULT_FAILURE;
    }
    ...
    if (onoff == SHDISP_DEV_REQ_ON) {
        switch (shdisp_pm_ctx.panel_status.power_status) {
        case SHDISP_DEV_STATE_OFF:
            ret = shdisp_pm_panel_set_active(mode);
            if (ret != SHDISP_RESULT_SUCCESS) {
                SHDISP_ERR("<RESULT_FAILURE> shdisp_pm_panel_set_active.");
                return SHDISP_RESULT_FAILURE;
            }
            shdisp_pm_ctx.panel_status.power_status = SHDISP_DEV_STATE_ON;
            break;
        ...
        switch (shdisp_pm_ctx.panel_status.power_status) {
        case SHDISP_DEV_STATE_ON:
            users_wk = shdisp_pm_ctx.panel_status.users;
            users_wk &= (unsigned long)(~(user & SHDISP_DEV_TYPE_PANEL_MASK));
            if (users_wk == SHDISP_DEV_TYPE_NONE) {
                ret = shdisp_pm_panel_set_standby(mode);
                if (ret != SHDISP_RESULT_SUCCESS) {
                    SHDISP_ERR("<RESULT_FAILURE> shdisp_pm_panel_set_standby.");
                    return SHDISP_RESULT_FAILURE;
                }
                shdisp_pm_ctx.panel_status.power_status = SHDISP_DEV_STATE_OFF;
            }
    ...
    if (onoff == SHDISP_DEV_REQ_ON) {
        shdisp_pm_ctx.panel_status.users |= (unsigned long)(user & SHDISP_DEV_TYPE_PANEL_MASK);
    } else if (onoff == SHDISP_DEV_REQ_OFF) {
        shdisp_pm_ctx.panel_status.users &= (unsigned long)(~(user & SHDISP_DEV_TYPE_PANEL_MASK));
    }

    SHDISP_TRACE("out [PANEL_PM]  users:0x%08X, power_status:%s",
            (int)shdisp_pm_ctx.panel_status.users,
            GET_DEV_STATE_STR(shdisp_pm_ctx.panel_status.power_status));
...
/* ------------------------------------------------------------------------- */
/* shdisp_pm_API_is_active_panel_user                                        */
/* ------------------------------------------------------------------------- */
int shdisp_pm_API_is_active_panel_user(unsigned int user)
{
    if((user & shdisp_pm_ctx.panel_status.users) != 0) {
        return SHDISP_DEV_STATE_ON;
    } else {
        return SHDISP_DEV_STATE_OFF;
    }
...
void shdisp_pm_API_power_manager_users_dump(void)
{
    ...
    printk("[SHDISP] [PANEL_PM]  users:0x%08X, power_status:%s\n",
            (int)shdisp_pm_ctx.panel_status.users,
            GET_DEV_STATE_STR(shdisp_pm_ctx.panel_status.power_status));
    ...
    printk("[SHDISP] [LPSOC_PM]  users:0x%08X, power_status:%s\n",
            (int)lpsoc_status.users,
            GET_DEV_STATE_STR(lpsoc_status.power_status));
...
#if defined(USE_LINUX) || defined(SHDISP_APPSBL)
/* ------------------------------------------------------------------------- */
/* shdisp_pm_panel_set_active                                                */
/* ------------------------------------------------------------------------- */
static int shdisp_pm_panel_set_active(int mode)
{
    int ret;

    SHDISP_TRACE("in panel_status:%d, mode=%d", shdisp_pm_ctx.panel_status.power_status, mode);

    ret = shdisp_panel_API_power_on(mode);
...
static int shdisp_pm_panel_set_standby(int mode)
{
    ...
    ret = shdisp_panel_API_power_off(mode);
