...
{
	struct shdisp_boot_context* shdisp_smem;
	...
	if( sh_smem != 0 )
	{
		shdisp_smem = (struct shdisp_boot_context*)sh_smem->shdisp_data_buf;

		if(copy_to_user((int __user *)arg, &(shdisp_smem->upper_unit_is_connected), sizeof(int)) != 0)
		{
