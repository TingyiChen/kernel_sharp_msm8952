...
{
	printk("[BSP-SERI]:%s() start",__func__ );
	if(!msm_hsl_port) {
	...
	printk("[BSP-SERI]:%s() end",__func__ );
...
static int msm_hsl_pinctrl_select(struct uart_port *port, bool flg)
{
	...
	printk("[BSP-SERI]:%s() flg=%d",__func__,flg );
